...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(buildUpstreamUrl(`/api/units/${id}/admins/assign/`), {
      method: "POST",
    ...
    return NextResponse.json(
      { message: error instanceof Error && error.message ? error.message : "Unable to assign unit admin right now." },
      { status: 502 },
    );
  }
}
