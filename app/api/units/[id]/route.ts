...
async function forwardRequest(
  ...
  const body =
    method === "PUT" || method === "PATCH" ? JSON.stringify(await request.json()) : undefined;

  const response = await fetch(
    buildUpstreamUrl(`/api/units/${id}/`, request.nextUrl.searchParams),
    {
      method,
...
export async function GET(
  ...
  try {
    ...
    return await forwardRequest(request, id, "GET");
  } catch (error) {
...
export async function PUT(
  ...
  try {
    const { id } = await context.params;
    return await forwardRequest(request, id, "PUT", true);
  } catch (error) {
    return NextResponse.json(
      {
        message:
...
export async function PATCH(
  ...
  try {
    const { id } = await context.params;
    return await forwardRequest(request, id, "PATCH", true);
  } catch (error) {
    return NextResponse.json(
      {
        message:
...
export async function DELETE(
  ...
  try {
    const { id } = await context.params;
    return await forwardRequest(request, id, "DELETE");
  } catch (error) {
    return NextResponse.json(
      {
        message:
          error instanceof Error && error.message
            ? error.message
            : "Unable to delete unit right now.",
      },
      { status: 502 },
    );
  }
