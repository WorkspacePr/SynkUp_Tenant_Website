...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      buildUpstreamUrl(`/api/organizations/${organizationId}/onboarding/profile/`),
      {
        method: "GET",
    ...
    return NextResponse.json(
      {
        message:
          error instanceof Error && error.message
            ? error.message
            : "Unable to load organisation profile right now.",
      },
      { status: 502 },
    );
  }
...
export async function PATCH(
  ...
  try {
    ...
    const response = await fetch(
      ...
      {
        method: "PATCH",
        headers: buildForwardHeaders(request, !isMultipart),
    ...
    return NextResponse.json(
      {
        message:
          error instanceof Error && error.message
            ? error.message
            : "Unable to update organisation profile right now.",
      },
      { status: 502 },
    );
  }
