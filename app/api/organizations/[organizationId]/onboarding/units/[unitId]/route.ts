...
export async function PATCH(
  request: NextRequest,
  context: { params: Promise<{ organizationId: string; unitId: string }> },
) {
  try {
    const { organizationId, unitId } = await context.params;
    const body = await request.json();

    const response = await fetch(
      buildUpstreamUrl(`/api/organizations/${organizationId}/onboarding/units/${unitId}/`),
      {
        method: "PATCH",
