...
export async function PATCH(
  request: NextRequest,
  context: { params: Promise<{ organizationId: string; audienceId: string }> },
) {
  try {
    const { organizationId, audienceId } = await context.params;
    ...
    return NextResponse.json(
      {
        message:
          error instanceof Error && error.message
            ? error.message
            : "Unable to update audience right now.",
      },
      { status: 502 },
    );
  }
    ...
    return NextResponse.json(
      {
        message:
          error instanceof Error && error.message
            ? error.message
            : "Unable to archive audience right now.",
      },
      { status: 502 },
    );
  }
