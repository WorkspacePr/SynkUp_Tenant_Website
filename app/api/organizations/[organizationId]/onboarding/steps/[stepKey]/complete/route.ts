...
export async function POST(
  request: NextRequest,
  context: { params: Promise<{ organizationId: string; stepKey: string }> },
) {
  try {
    const { organizationId, stepKey } = await context.params;
    ...
    return NextResponse.json(
      {
        message:
          error instanceof Error && error.message
            ? error.message
            : "Unable to complete onboarding step right now.",
      },
      { status: 502 },
    );
  }
