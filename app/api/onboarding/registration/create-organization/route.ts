...
function getCreateOrganizationPath() {
  return (
    process.env.CREATE_ORGANIZATION_PATH ??
    "/api/onboarding/registration/create-organization/"
  );
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(buildUpstreamUrl(getCreateOrganizationPath()), {
      method: "POST",
      headers: {
