...
import {
  ...
  type TenantUnitOverviewSummaryCards,
...
function extractAdminSummary(value: unknown): string[] {
  ...
  if (Array.isArray(value)) {
    return value
      .flatMap((item) => extractAdminSummary(item))
      .filter(Boolean);
  }
  ...
  if (value && typeof value === "object") {
    ...
    if (typeof record.full_name === "string" && record.full_name.trim().length > 0) {
...
function mapUnit(unit: TenantUnitListItem): UnitSummary {
  ...
  const admins = extractAdminSummary(unit.assigned_admin_summary);
...
function createOverviewMetrics(summaryCards: TenantUnitOverviewSummaryCards): OverviewMetric[] {
  return [
    { ...OVERVIEW_METRICS[0], value: formatCount(summaryCards.totalUnits) },
    { ...OVERVIEW_METRICS[1], title: "Active Units", value: formatCount(summaryCards.activeUnits) },
    { ...OVERVIEW_METRICS[2], title: "Draft Units", value: formatCount(summaryCards.draftUnits) },
    { ...OVERVIEW_METRICS[3], title: "Archived Units", value: formatCount(summaryCards.archivedUnits), icon: OVERVIEW_METRICS[3].icon, titleClassName: undefined, valueClassName: undefined },
  ];
}
...
export function UnitsWorkspace({
  ...
  const [overviewUnits, setOverviewUnits] = useState<UnitSummary[]>([]);
  ...
  const loadAssignedAdminsForUnit = async (unitId: number) => {
  ...
  useEffect(() => {
    ...
    const loadOverview = async () => {
      ...
      setOverviewError("");
      const result = await getTenantUnitsOverview();
      ...
      setOverviewUnits(result.data.data.featuredUnits.map(mapUnit));
      setOverviewMetrics(createOverviewMetrics(result.data.data.summaryCards));
      setOverviewPlanUsage(result.data.planUsage ?? null);
      setOverviewLoading(false);
    };
  ...
  }, [hasLoadedUnits, page, query, ordering]);
  ...
  useEffect(() => {
    ...
    void loadDetail();
    void loadAssignedAdminsForUnit(unitId);
  ...
  const overviewUnitCount = overviewPlanUsage?.units_used ?? overviewUnits.length;
  const overviewUnitLimit = overviewPlanUsage?.unit_limit ?? PLAN_UNIT_LIMIT;
  const overviewUnitUsageText =
    overviewPlanUsage && overviewUnitLimit <= 0
      ? `${overviewUnitCount} / Unlimited`
      : UNLIMITED_UNITS
        ? `${overviewUnitCount} / Unlimited`
        : `${overviewUnitCount} / ${overviewUnitLimit}`;
  const overviewPlanLimitReached =
    overviewPlanUsage?.creation_blocked ??
    (!UNLIMITED_UNITS && overviewUnitCount >= PLAN_UNIT_LIMIT);
        ...
        {view === "detail" ? (
          detailLoading && !primaryUnit ? (
            ...
            <UnitDetail
              darkMode={darkMode}
              unit={primaryUnit}
              audiences={AUDIENCE_LOOKUP[primaryUnit.id] ?? []}
              assignedAdmins={assignedAdmins}
              availableUnits={units.filter((unitValue) => unitValue.id !== primaryUnit.id)}
